module pipelined_cpu_forwarding #(
    ...
    wire redirect;
    wire advance = (CLOCK_ENABLE == 0) ? 1'b1 : clock_en;
    wire[31:0] redirect_loc;
    wire stall;
    wire[31:0] forward_rs1, forward_rs2;
    assign pc_4 = pc + 32'b100;
    assign pc_next = advance ? (redirect ? redirect_loc : (stall ? pc : pc_4)) : pc;
    ...
    regfile u_regfile(
        ...
        .rs1_data(rs1_data),
        .rs2_data(rs2_data),
        .write_enable(wrb_reg_write && advance),
        .rd_addr(wrb_rd_addr),
        .rd_data(wrb_rd_data)
    );
    ...
    always @(posedge clock or negedge rst_inv) begin
        if(!rst_inv) begin
            ...
            if(redirect || stall) begin
                ...
                dec_exe_rs1_data <= (mem_wrb_reg_write && (mem_wrb_rd_addr != 5'h0) && 
                    (mem_wrb_rd_addr == rs1)) ? wrb_rd_data : rs1_data; 
                dec_exe_rs2_data <= (mem_wrb_reg_write && (mem_wrb_rd_addr != 5'h0) &&
                    (mem_wrb_rd_addr == rs2)) ? wrb_rd_data : rs2_data;
                dec_exe_imm <= imm;
    ...
    wire[31:0] alu_a = dec_exe_alu_in_a ? dec_exe_pc : forward_rs1;
    wire[31:0] alu_b = dec_exe_alu_in_b ? dec_exe_imm : forward_rs2;
    ...
    data_mem u_datamem(
        .clock(clock),
        .write_enable(exe_mem_mem_write && advance),
    ...
    wire[1:0] forward_a = (exe_mem_reg_write && (exe_mem_rd_addr != 5'h0) && 
        (exe_mem_rd_addr == dec_exe_rs1_addr)) ? 2'b01 :
        (mem_wrb_reg_write && (mem_wrb_rd_addr != 5'h0) && 
        (mem_wrb_rd_addr == dec_exe_rs1_addr)) ? 2'b10 : 2'b00;
    
    wire[1:0] forward_b = (exe_mem_reg_write && (exe_mem_rd_addr != 5'h0) &&
        (exe_mem_rd_addr == dec_exe_rs2_addr)) ? 2'b01 :
        (mem_wrb_reg_write && (mem_wrb_rd_addr != 5'h0) && 
        (mem_wrb_rd_addr == dec_exe_rs2_addr)) ? 2'b10 : 2'b00;

    wire[31:0] exe_mem_result = (exe_mem_result_sel == 2'b00) ? exe_mem_alu_result :
        (exe_mem_result_sel == 2'b10) ? exe_mem_pc_4 :
        (exe_mem_result_sel == 2'b11) ? exe_mem_imm : exe_mem_alu_result;

    assign forward_rs1 = (forward_a == 2'b01) ? exe_mem_result : 
        (forward_a == 2'b10) ? wrb_rd_data : dec_exe_rs1_data;
    
    assign forward_rs2 = (forward_b == 2'b01) ? exe_mem_result :
        (forward_b == 2'b10) ? wrb_rd_data : dec_exe_rs2_data;

    wire load_stall = dec_exe_mem_read && (dec_exe_rd_addr != 5'h0) && 
